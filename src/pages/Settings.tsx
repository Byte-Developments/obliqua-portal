...
function Settings() {
  ...
  const fetchSettings = async () => {
    try {
      const response = await axios.get('https://199.85.208.153:3000/api/users/settings');
  ...
  const handleAvatarUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      await axios.post('https://199.85.208.153:3000/api/users/avatar', formData, {
  ...
  const updateSettings = async (newSettings: Partial<UserSettings>) => {
    ...
    try {
      ...
      const response = await axios.put('https://199.85.208.153:3000/api/users/settings', updatedSettings);
      setSettings(response.data);
      setError(null);
    } catch (err: any) {
      ...
      <section className="glass-card rounded-xl p-6 mb-6">
        ...
        <div className="mb-6">
          <div className="flex items-center space-x-4">
            {user?.avatar_hash ? (
              <img
                src={`https://199.85.208.153:3000/api/avatars/${user.avatar_hash}`}
                alt="User avatar"
